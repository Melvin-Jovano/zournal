function API() {
    ...
    $("#noInternet").hide();
    toggleLoader();
    fd.append("username", $("#userN").val());
    fetch("https://melpinchan.000webhostapp.com/configBeta/data.php", {method : "POST", body : fd})
        .then(res => res.json())
        .then((data) => {
            toggleLoader();
            $("#signal").html("sync");
            $("#signalStatus").html("Online&nbsp;");
            ...
            M.toast({ html: "<i class='medium material-icons'>notifications_active</i>&nbsp;" + data.data.length + " Note's Found", classes:"toast red rounded left" });
            ...
            for (var i = 0; i < data.data.length; i++) {
                ...
                div_card_content_title.setAttribute("class", "title"+data.data[i].id);
                ...
                div_card_content_name.setAttribute("class", "name"+data.data[i].id);
                ...
                div_card_b_btn_icon.setAttribute("class", "material-icons large delt");
                ...
                first_col_img.setAttribute("class", "responsive-img img-modal");
                ...
                messages.setAttribute("class", "messages container note"+ data.data[i].id);
        ...
        .catch((err) => {
            if (err == "TypeError: Failed to fetch") {
                M.toast({ html: "<i class='medium material-icons'>error</i>&nbsp;No Internet Connection", classes: "toast red rounded left" })
                toggleLoader();
                $("#noInternet").show();
                $("#signal").html("sync_disabled");
                $("#signalStatus").html("Offline&nbsp;");
            }  else {
                M.toast({ html: "<i class='medium material-icons'>error</i>&nbsp;No Data Found", classes: "toast red rounded left" })
            }
        });
